...
class VagrantVmController:
    ...
    def vm_set_running(self):
        ...
        if self.get_vagrant_running_status() != 'running':
            ...
            has_changed = True 
        status = self.vm.conf()
        status.update({'OS': self.vm._run_vagrant_command(['ssh', '-c', "hostnamectl status"]).split('\n')})
        status.update({'RAM size': self.vm._run_vagrant_command(['ssh', '-c', "cat /proc/meminfo | grep MemTotal:"]).split('\n')[0:-1]}) 
